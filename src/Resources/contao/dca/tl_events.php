...
$GLOBALS['TL_DCA']['tl_events'] = [
    'config' => [        
		...
		//'onload_callback' => ['tl_events' => 'onloadCallback']
    ],
	...
	(
		...
		(
			...
			'panel_callback'          => array(
				'testpanel' => ['tl_events', 'addPanelData']
			)
		),
		'label' => array
		(
			...
			'showColumns'             => true,
			'label_callback'          => array('tl_events', 'addLabelData')
		),
		'global_operations' => array
		(
			'csv' => array(
                'href'      => '#',
                'button_callback'=> array('tl_events', 'downloadButton')
            )
			/*'all' => array
			(
				'href'                => 'act=select',
	...
	(
		...
		'default'                     => '{personal_legend},name,alias,avatar,sparten,zielgruppen,artists,linked_artists;
    ...
    'fields' => [
		...
		'disable' => [
			...
			'inputType'               => 'checkbox',
			'sql'                     => "char(1) NOT NULL default ''",
			//'save_callback'			  => [['tl_events', 'sendActivationMail']]
		],
		'readonly' => [
		...
		'sendmailsubject' => [
			...
			'inputType'				=> 'text',
			'save_callback'			=> [
				['tl_events', 'dontsaveempty']
			],
			'load_callback' 		=> [
				['tl_events', 'loadsubject']
			],
			'eval'					=> [
				'doNotSaveEmpty'	=> true
			],
		...
		'memberinfos' => [
            ...
            'input_field_callback' => [
				'tl_events', 'memberinfos'
			]
        ],
    ],
];

//$GLOBALS['TL_DCA']['tl_events']['list']['sorting']['fields'] = ['host_ort ASC'];
//$GLOBALS['TL_DCA']['tl_events']['list']['label']['fields'] = array('name', 'member_id', 'email', 'location_ort', 'tstamp');

class tl_events extends Backend {
	...
	public function addPanelData(DataContainer $dc) 
	{
		...
		if(isset($data['sorting']['tl_events'])) {
			if($data['sorting']['tl_events'] !== 'name') {
				$GLOBALS['TL_DCA']['tl_events']['list']['label']['fields'] = array('name', 'member_id', 'email', $data['sorting']['tl_events']);	
			}
		}
	}
	...
	public function generateAlias($varValue, DataContainer $dc)
	{
		...
		{
			return $this->Database->prepare("SELECT id FROM tl_events WHERE alias=? AND id!=?")->execute($alias, $dc->id)->numRows > 0;
		};
	...
	public function addLabelData($row, $label, DataContainer $dc, $args) 
	{
        ...
        $args[$key] = '<a href="https://'.Idna::decode(Environment::get('host')).'/detailansicht-event/'.$row['alias'].'.html" target="_blank" title="Im Frontend ansehen">'.$oldVar.'</a>'.$readonly;
	...
	public function loadsubject($varValue, DataContainer $dc) {
		return 'Ihre Veranstaltung wurde überprüft';
	}
	...
	public function loadmailtext($varValue, DataContainer $dc) {
		...
		$var = "Moin ".$objMember->firstname." ".$objMember->lastname."!\r\n\r\nIhre Veranstaltung im SE-KulturNetz wurde überprüft und freigeschaltet - herzlichen Glückwunsch!\r\n\r\nSie können Ihre Veranstaltung über folgenden Link erreichen:\r\nhttps://".Idna::decode(Environment::get('host'))."/detailansicht-event/".$dc->activeRecord->alias.".html\r\n\r\nLiebe Grüße\r\nIhr SE-KulturTeam";
