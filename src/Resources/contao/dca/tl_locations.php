...
$GLOBALS['TL_DCA']['tl_locations'] = [
	...
	(
		...
		(
			...
			'showColumns'             => true,
			'label_callback'          => array('tl_locations', 'addLabelData')
		),
		'global_operations' => array
		(
			'csv' => array(
                'href'      => '#',
                'button_callback'=> array('tl_locations', 'downloadButton')
            )
			/*'all' => array
			(
				'href'                => 'act=select',
	...
	(
		...
		{location_legend},barrierefreiheit,auftrittsflaeche,gesamtflaeche,buehnentyp,technik,sitzplaetze,stehplaetze;
    ...
    'fields' => [
		...
		'alias' => [
			...
			'save_callback' => array
			(
				array('tl_locations', 'generateAlias')
			),
		],
		'avatar' => [
            'inputType' => 'fileTree',
		...
		'barrierefreiheit' => [
			'label'					=> ['Barrierefreiheit', 'Informationen zur Barrierefreiheit der Location'],
			'inputType'               => 'text',
			'sql'                     => "varchar(255) NOT NULL default ''"
		],
		'auftrittsflaeche' => [
			'label'					=> ['Bühnen-Auftrittsfläche', ''],
			'inputType'               => 'text',
			'sql'                     => "varchar(255) NOT NULL default ''"
		],
		'gesamtflaeche' => [
		...
		'disable' => [
			...
			//'save_callback'			  => [['tl_locations', 'sendActivationMail']]
		],
		'comment' => [
		...
		'sendmailsubject' => [
			...
			'inputType'				=> 'text',
			'save_callback'			=> [
				['tl_locations', 'dontsaveempty']
			],
			'load_callback' 		=> [
				['tl_locations', 'loadsubject']
			],
			'eval'					=> [
				'doNotSaveEmpty'	=> true
			],
		...
		'memberinfos' => [
            ...
            'input_field_callback' => [
				'tl_locations', 'memberinfos'
			]
        ],
    ],
];

class tl_locations extends Backend {
	...
	public function downloadButton($arrRow, $href, $label, $title, $icon) {
        return '
		<a href="./files/se-kultur.de/code/csvLocations.php" style="display:inline-block;margin-left: 15px;">
	...
	public function generateAlias($varValue, DataContainer $dc)
	{
		...
		{
			return $this->Database->prepare("SELECT id FROM tl_locations WHERE alias=? AND id!=?")->execute($alias, $dc->id)->numRows > 0;
		};
	...
	public function addLabelData($row, $label, DataContainer $dc, $args) 
	{
        ...
        $args[$key] = '<a href="https://'.Idna::decode(Environment::get('host')).'/detailansicht-locationprofil/'.$row['alias'].'.html" target="_blank" title="Im Frontend ansehen">'.$oldVar.'</a>';
	...
	public function sendActivationMail($varValue, DataContainer $dc) 
	{
		/*if ($dc->activeRecord->disable != $varValue && $varValue != '1') {
			...
			$objEmail->subject = 'Ihr SE-KulturNetz Locationprofil wurde aktiviert';
			$objEmail->text = "Moin ".$objMember->firstname." ".$objMember->lastname."!\r\n\r\nIhr Locationprofil im SE-KulturNetz wurde aktiviert - herzlichen Glückwunsch!\r\n\r\nEs ist nun über unsere Webseite öffentlich erreichbar:\r\nhttps://".Idna::decode(Environment::get('host'))."/detailansicht-locationprofil/".$dc->activeRecord->alias.".html\r\n\r\nLiebe Grüße\r\nIhr SE-KulturTeam";
	...
	public function loadsubject($varValue, DataContainer $dc) {
		return 'Ihr SE-KulturNetz Locationprofil wurde aktiviert';
	}
