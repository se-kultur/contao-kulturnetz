...
$GLOBALS['TL_DCA']['tl_artists'] = [
	...
	(
		...
		(
			...
			'showColumns'             => true,
			'label_callback'          => array('tl_artists', 'addLabelData')
		),
		'global_operations' => array
		(
			'csv' => array(
                'href'      => '#',
                'button_callback'=> array('tl_artists', 'downloadButton')
            )
			/*'all' => array
			(
				'href'                => 'act=select',
	...
	(
		...
		'default'                     => '{personal_legend},name,alias,avatar,sparten,themen;
    ...
    'fields' => [
		...
		'alias' => [
			...
			'save_callback' => array
			(
				array('tl_artists', 'generateAlias')
			),
		],
		'avatar' => [
            'inputType' => 'fileTree',
		...
		'disable' => [
			...
			//'save_callback'			  => [['tl_artists', 'sendActivationMail']]
		],
		'comment' => [
		...
		'sendmailsubject' => [
			...
			'inputType'				=> 'text',
			'save_callback'			=> [
				['tl_artists', 'dontsaveempty']
			],
			'load_callback' 		=> [
				['tl_artists', 'loadsubject']
			],
			'eval'					=> [
				'doNotSaveEmpty'	=> true
			],
...
class tl_artists extends Backend {
	...
	public function downloadButton($arrRow, $href, $label, $title, $icon) {
        return '
		<a href="./files/se-kultur.de/code/csvArtists.php" style="display:inline-block;margin-left: 15px;">
	...
	public function generateAlias($varValue, DataContainer $dc)
	{
		...
		{
			return $this->Database->prepare("SELECT id FROM tl_artists WHERE alias=? AND id!=?")->execute($alias, $dc->id)->numRows > 0;
		};
	...
	public function addLabelData($row, $label, DataContainer $dc, $args) 
	{
        ...
        $args[$key] = '<a href="https://'.Idna::decode(Environment::get('host')).'/detailansicht-kuenstlerprofil/'.$row['alias'].'.html" target="_blank" title="Im Frontend ansehen">'.$oldVar.'</a>';
	...
	public function sendActivationMail($varValue, DataContainer $dc) 
	{
		/*if ($dc->activeRecord->disable != $varValue && $varValue != '1') {
			...
			$objEmail->subject = 'Ihr SE-KulturNetz Künstler:innenprofil wurde aktiviert';
			$objEmail->text = "Moin ".$objMember->firstname." ".$objMember->lastname."!\r\n\r\nIhr Künstler:innenprofil im SE-KulturNetz wurde aktiviert - herzlichen Glückwunsch!\r\n\r\nEs ist nun über unsere Webseite öffentlich erreichbar:\r\nhttps://".Idna::decode(Environment::get('host'))."/detailansicht-kuenstlerprofil/".$dc->activeRecord->alias.".html\r\n\r\nLiebe Grüße\r\nIhr SE-KulturTeam";
	...
	public function loadsubject($varValue, DataContainer $dc) {
		return 'Ihr SE-KulturNetz Künstler:innenprofil wurde aktiviert';
	}
	...
	public function mailing($varValue, DataContainer $dc) {
		if ($varValue == '1') {
			...
			$this->log(
				'Bestätigungsmail wurde an '.$objMember->email.' gesendet (tl_artists)', __METHOD__, TL_EMAIL
			);
		}	
